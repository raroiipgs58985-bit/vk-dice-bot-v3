...
class EconomyManager:
    ...
    def _load_from_database(self) -> None:
        for row in self.db.load_players():
            p = Player(vk_id=int(row["vk_id"]), name=str(row["name"]), balance=int(row["balance"]),
        ...
        if payload:
            ...
            for item in payload.get("bets", []):
                b = Bet(int(item["user_id"]), str(item["outcome"]), int(item["amount"]))
                event.bets[b.user_id] = b
    ...
    def get_or_create_player(self, user_id: int, name: str) -> Player:
        ...
        if p is None:
            p = Player(user_id, name, self.starting_balance)
            self.players[user_id] = p
            self.db.save_player(p)
            self._log(p, f"+{self.starting_balance} — стартовый капитал")
    ...
    def _resolve_target_id(self, tail: str, message: dict[str, Any]) -> int | None:
        m = MENTION_RE.search(tail) or ID_RE.search(tail)
        if m:
    ...
    def _casino_result(self, p: Player, stake: int, payout: int, game: str, details: str) -> str:
        p.balance -= stake
        p.balance += payout
        profit = payout - stake
        p.casino_games += 1
        p.casino_wagered += stake
        p.casino_profit += profit
        if profit > 0:
            p.casino_wins += 1
            p.biggest_win = max(p.biggest_win, payout)
        else:
            p.casino_losses += 1
        self._save(p)
        self._log(p, f"{profit:+d} — казино: {game}")
        tx = self.db.add_transaction(kind="casino", amount=abs(profit), reason=game, source_vk_id=p.vk_id if profit < 0 else None,
                                     target_vk_id=p.vk_id if profit > 0 else None, metadata={"stake": stake, "payout": payout, "details": details})
    ...
    def handle(self, *, text: str, user_id: int, user_name: str, message: dict[str, Any], vk: Any) -> str | None:
        ...
        if lower == "[жалование":
            now = time.time(); elapsed = now-player.last_salary_at
            ...
            player.balance += amount; player.last_salary_at = now
            self._save(player); self._log(player, f"+{amount} — жалование")
        ...
        if lower.startswith("[перевести "):
            ...
            player.balance -= amount; target.balance += amount
            self._save(player, target); self._log(player, f"-{amount} — перевод {target.name}"); self._log(target, f"+{amount} — перевод от {player.name}")
            tx = self.db.add_transaction(kind="transfer", amount=amount, reason="Перевод игроку", actor_vk_id=user_id, source_vk_id=user_id, target_vk_id=target_id)
            return f"✅ Переведено {amount} тронов игроку {target.name}.\nВаш баланс: {player.balance}\nОперация: {tx[:8]}"
        ...
        for prefix, game in (("[монетка ", "Монетка"), ("[кости ", "Кости"), ("[слоты ", "Слоты")):
            if lower.startswith(prefix):
                ...
                if stake > player.balance:
                    return f"Недостаточно тронов. Ваш баланс: {player.balance}"
                if game == "Монетка":
                    win = random.random() < 0.48; side = random.choice(("орёл", "решка")); payout = stake*2 if win else 0
                    return self._casino_result(player, stake, payout, game, f"Выпало: {side}")
                if game == "Кости":
        ...
        if lower in ("[банк", "[ставки"):
            ...
            lines = []
            for i,o in enumerate(self.event.outcomes,1):
                bets=[b for b in self.event.bets.values() if b.outcome==o]
                lines.append(f"{i}. {o} — {len(bets)} ставок, {sum(b.amount for b in bets)} тронов")
        ...
        if lower.startswith("[ставка "):
            ...
            tail=self._clean(text[len("[ставка"):]); parts=tail.split(maxsplit=1)
            if len(parts)!=2 or not parts[0].isdigit():
                return "Формат: [ставка 25 Победа"
            amount=int(parts[0]); outcome=self._find_outcome(self.event,parts[1])
            if amount<=0 or amount>player.balance:
                return f"Недостаточно тронов. Ваш баланс: {player.balance}"
            if outcome is None:
                return "Не удалось определить исход."
            player.balance-=amount; player.bets_count+=1; player.total_bet+=amount; self.event.bets[user_id]=Bet(user_id,outcome,amount)
            self._save(player); self._log(player,f"-{amount} — ставка: {outcome}"); self.db.save_event(self.event)
            self.db.add_transaction(kind="event_bet",amount=amount,reason=outcome,source_vk_id=user_id,metadata={"event":self.event.title})
        ...
        if lower.startswith("[событие "):
            ...
            outcomes=list(dict.fromkeys(chunks[1:])); self.event=BettingEvent(chunks[0],outcomes); self.db.save_event(self.event)
        ...
        if lower == "[закрыть ставки":
            ...
            if not self.event:
                return "Активного события нет."
            self.event.bets_open=False; self.db.save_event(self.event); return f"🔒 Ставки закрыты. Банк: {self.event.bank}"
        if lower.startswith("[итог "):
            ...
            event=self.event; bank=event.bank; winners=[b for b in event.bets.values() if b.outcome==winning]; payouts={}
            if winners:
                total=sum(b.amount for b in winners); payouts={b.user_id:bank*b.amount//total for b in winners}
                rem=bank-sum(payouts.values())
                for b in sorted(winners,key=lambda x:(-x.amount,x.user_id)):
                    if rem<=0:
                        break
                    payouts[b.user_id]+=1; rem-=1
            lines=[]
            for b in event.bets.values():
                p=self.players.get(b.user_id)
                if not p:
                    continue
                if b.user_id in payouts:
                    pay=payouts[b.user_id]; p.balance+=pay; p.wins+=1; p.total_won+=pay; p.biggest_win=max(p.biggest_win,pay); self._log(p,f"+{pay} — выигрыш: {winning}"); lines.append(f"{p.name} — +{pay}")
                else:
                    p.losses+=1; p.total_lost+=b.amount
                self._save(p)
            self.event=None; self.db.save_event(None)
            return f"🏁 Итог: {winning}\nБанк: {bank}\n\n"+("\n".join(lines) if lines else "Победителей нет.")
        if lower in ("[отмена события", "[сброс ставок"):
            ...
            if not self.event:
                return "Активного события нет."
            refunded=0
            for b in self.event.bets.values():
                p=self.players.get(b.user_id)
                if p:
                    p.balance+=b.amount; refunded+=b.amount; self._save(p); self._log(p,f"+{b.amount} — возврат ставки")
            title=self.event.title; self.event=None; self.db.save_event(None); return f"↩ Событие «{title}» отменено. Возвращено: {refunded}"
        ...
        for command,mode in (("[выдать ","give"),("[забрать ","take"),("[установить ","set")):
            if lower.startswith(command):
                ...
                if denied:
                    return denied
                tail=self._clean(text[len(command.rstrip()):]); target_id=self._resolve_target_id(tail,message); amount=self._extract_last_amount(tail)
                if target_id is None or amount is None:
                    return "Укажите игрока и сумму."
                target=self.get_or_create_player(target_id,self._get_vk_name(vk,target_id)); old=target.balance
                target.balance = old+amount if mode=="give" else max(0,old-amount) if mode=="take" else max(0,amount)
                delta=target.balance-old; self._save(target); self._log(target,f"{delta:+d} — изменение администратором")
                tx=self.db.add_transaction(kind=f"admin_{mode}",amount=abs(delta),reason="Административное изменение",actor_vk_id=user_id,
                                           source_vk_id=target_id if delta<0 else None,target_vk_id=target_id if delta>0 else None,metadata={"old":old,"new":target.balance})
        ...
        if lower.startswith("[операции") or lower.startswith("[история игрока"):
            ...
            lines=[f"{str(r['id'])[:8]} | {r['kind']} | {r['amount']} | {r['reason']}" for r in rows]
        ...
        if lower.startswith("[операция "):
            ...
            r=self.db.get_transaction(match.group(1))
            return "Операция не найдена." if not r else json.dumps(dict(r),ensure_ascii=False,default=str,indent=2)
        if lower.startswith("[откатить "):
            ...
            amount=int(r["amount"]); source=r.get("source_vk_id"); target=r.get("target_vk_id")
            source_p=self.players.get(int(source)) if source else None; target_p=self.players.get(int(target)) if target else None
            ...
            reverse=self.db.add_transaction(kind="reversal",amount=amount,reason=f"Откат {r['id']}",actor_vk_id=user_id,
                                            source_vk_id=target,target_vk_id=source)
            self.db.mark_reversed(str(r["id"]),reverse)
            return f"↩ Операция отменена. Новый ID: {reverse}"
        if lower == "[экспорт экономики":
            denied=self._admin_required(user_id)
            if denied:
                return denied
            snapshot=self.db.export_snapshot(); data=json.dumps(snapshot,ensure_ascii=False,default=str)
        ...
        if lower in ("[экономика","[диагностика базы"):
            ...
            if denied:
                return denied
            total=sum(p.balance for p in self.players.values()); event="нет" if not self.event else f"{self.event.title}, банк {self.event.bank}"
            return f"⚙ Экономика Кубятни\nИгроков: {len(self.players)}\nТронов: {total}\nБД: {'подключена' if self.db.enabled else 'не подключена'}\nСхема: {self.db.schema_version}\nСобытие: {event}"
        ...
        if lower == "[сброс тронов":
            ...
            if denied:
                return denied
            self.players.clear(); self.event=None; self.db.reset_all(); return "♻ Экономика полностью сброшена."
        return None
