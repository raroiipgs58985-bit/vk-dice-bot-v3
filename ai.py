...
class OpenRouterClient:
    ...
    def plan_queries(self, question: str) -> QueryPlan:
        system = (
            ...
            "и имена сущностей. Также предложи 3-8 вероятных точных заголовков статей "
            "на английском языке. Для терминов Warhammer 40,000 сохраняй официальные "
        ...
        title_candidates = self._clean_string_list(
            payload.get("title_candidates"), 8, 120
        )
        if not title_candidates:
            title_candidates = entities[:8]
        if not queries:
