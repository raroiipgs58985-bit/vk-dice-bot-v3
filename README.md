...
MEDIAWIKI_API_URL=https://wh40k.lexicanum.com/mediawiki/
...
Lexicanum may reject automated search/API requests from cloud IP addresses.
This version first uses the published `/mediawiki/` entry point, then tries the
normal HTML search, direct article title candidates, and finally the public Jina
Reader as a read-only fallback. Disable the latter with
`JINA_READER_FALLBACK=false`.
