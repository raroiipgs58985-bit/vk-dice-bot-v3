...
class SiteCrawler:
    ...
    def _fetch_document_internal(
        ...
        try:
            ...
            if self.settings.jina_reader_fallback:
                document = self._fetch_via_jina_reader(url)
    ...
    def _fetch_via_jina_reader(self, url: str) -> PageDocument | None:
        with self._jina_lock:
            if self._jina_fallback_count >= self.settings.jina_max_fallback_pages:
                return None
        ...
        title = ""
        match = re.search(r"(?m)^Title:\s*(.+)$", body)
        if match:
        ...
        if not title:
            title = PurePosixPath(urlsplit(url).path).name.replace("_", " ") or url
        text = _SPACE_RE.sub(" ", body).strip()
        if len(text) < 80:
            return None
        return PageDocument(url, title[:300], text, "text/markdown")
